...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const guideData = [
      ...
      ['공급가', '선택', '매입 원가 (관리자 전용, 구매자에게 미노출)', '40000'],
      ['재고수량', '필수(*)', '0 이상의 정수', '100'],
      ['SKU(상품코드)', '선택', '고유한 상품 관리 코드', 'BT-X1-BLK'],
      ['상품 간단설명', '선택', '상품 목록에 표시될 1~2줄 설명 (최대 200자)', '고품질 블루투스 5.3 이어폰'],
      ...
      ['', '', '', ''],
      ['[옵션 입력 방법]', '', '', ''],
      ['1. 옵션이 있는 상품은 "옵션입력" 시트에 입력하세요.', '', '', ''],
      ['2. "연결상품명"은 "상품입력" 시트의 상품명과 정확히 일치해야 합니다.', '', '', ''],
      ['3. 한 상품에 옵션 조합이 여러 개면 여러 행으로 입력하세요.', '', '', ''],
      ['4. 옵션가격이 비어 있으면 상품 기본 판매가를 사용합니다.', '', '', ''],
      ['5. 옵션이 없는 상품은 "옵션입력" 시트에 넣지 않아도 됩니다.', '', '', ''],
    ];
    ...
    const optExample1 = ['프리미엄 블루투스 이어폰 X1', '색상', '블랙', '', '', '59000', '50', 'BT-X1-BLK'];
    const optExample2 = ['프리미엄 블루투스 이어폰 X1', '색상', '화이트', '', '', '59000', '30', 'BT-X1-WHT'];
    const optExample3 = ['오가닉 코튼 라운드넥 티셔츠', '색상', '화이트', '사이즈', 'M', '35000', '80', 'CT-RN-WHT-M'];
    const optExample4 = ['오가닉 코튼 라운드넥 티셔츠', '색상', '화이트', '사이즈', 'L', '35000', '70', 'CT-RN-WHT-L'];
    const optExample5 = ['오가닉 코튼 라운드넥 티셔츠', '색상', '블랙', '사이즈', 'L', '37000', '60', 'CT-RN-BLK-L'];
    ...
    XLSX.utils.book_append_sheet(wb, ws4, '옵션입력');
